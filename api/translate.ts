...
const SYSTEM_PROMPT = (sourceLang: string, targetLang: string) =>
  `Translate from ${sourceLang} to ${targetLang}. Output translation only. Always return the translated text. Never return an empty response. If the source and target languages are the same, return the original text unchanged.`;
