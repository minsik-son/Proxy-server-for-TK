...
const SYSTEM_PROMPT = (tone, language, length, replyContext) => {
...
// ── Gemini ──────────────────────────────────────────
async function composeWithGemini(prompt, tone, language, length, replyContext) {
...
// ── OpenAI (fallback) ───────────────────────────────
async function composeWithOpenAI(prompt, tone, language, length, replyContext) {
...
async function composeMessage(prompt, tone, language, length, replyContext) {
...
async function handler(req) {
    ...
    try {
        ...
        if (prompt.length > MAX_PROMPT_LENGTH) {
            return Response.json({ error: `prompt exceeds maximum length of ${MAX_PROMPT_LENGTH}` }, { status: 400, headers });
        }
        const validTones = [
        ...
        if (safeCount > 1) {
            ...
            const promises = Array.from({ length: safeCount }, () => composeMessage(prompt.trim(), safeTone, safeLang, safeLength, safeReplyContext));
        ...
        const { message, engine } = await composeMessage(prompt.trim(), safeTone, safeLang, safeLength, safeReplyContext);
