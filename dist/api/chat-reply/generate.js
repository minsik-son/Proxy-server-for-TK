...
const TONE_INSTRUCTIONS = {
    casual: "Write in casual, informal tone. Use relaxed, conversational language.",
    formal: "Write in polite, formal tone. Use respectful and professional language.",
    polished: "Write in clean, well-structured style with proper punctuation.",
    friendly: "Write in warm, friendly tone. Be approachable and personable. Use emojis sparingly.",
    empathetic: "Write with warmth and emotional understanding. Show genuine care, empathy, and compassion.",
    confident: "Write with assertiveness and authority. Be direct, decisive, and self-assured without being arrogant.",
    witty: "Write with clever humor and wordplay. Be entertaining and sharp but keep the core message clear.",
    persuasive: "Write to convince and influence. Use logical arguments, compelling language, and strategic framing.",
    enthusiastic: "Write with energy, excitement, and positivity. Express genuine enthusiasm. Use exclamation marks sparingly but effectively.",
    apologetic: "Write with sincerity and remorse. Acknowledge the issue, take responsibility, and express willingness to make things right.",
    social: "Write in trendy, casual social media style. Use Gen-Z language patterns, appropriate emojis, abbreviations, and hashtag-ready phrasing.",
    professional: "Write with competence and strategic authority. Imply expertise and thought leadership. Distinct from formal — focus on substance over politeness.",
    polite: "Write in polite, courteous tone. Show respect and consideration. Use please, thank you, and other polite expressions naturally.",
    cool: "Write in cool, laid-back tone. Be effortlessly smooth and collected. Don't try too hard — let the style flow naturally.",
    romantic: "Write with warmth, affection, and romantic undertones. Be tender and emotionally expressive without being over-the-top.",
    direct: "Write with clarity and directness. Get straight to the point. No fluff, no hedging — say exactly what you mean.",
    thoughtful: "Write with depth and reflection. Show careful consideration of the topic. Be measured, nuanced, and contemplative.",
};
// ── v3.4 톤 매핑: iOS → Server ─────────────────────
// iOS toneAPIValues: ["Friendly", "Polite", "Cool", "Witty", "Professional", "Romantic", "Direct", "Thoughtful"]
// 대소문자 통일 + 누락 톤 매핑
const TONE_ALIAS_MAP = {
    friendly: "friendly",
    polite: "polite",
    cool: "cool",
    witty: "witty",
...
const SYSTEM_PROMPT = (tone, styleVariation, direction) => {
...
// ── Gemini ──────────────────────────────────────────
async function generateReplyWithGemini(context, tone, styleVariation, direction) {
...
// ── OpenAI (fallback) ───────────────────────────────
async function generateReplyWithOpenAI(context, tone, styleVariation, direction) {
...
async function generateReply(context, tone, styleVariation, direction) {
...
async function handler(req) {
    ...
    try {
        ...
        if (!context || typeof context !== "string" || context.trim().length === 0) {
            return Response.json({ error: "context is required" }, { status: 400, headers });
        }
        if (context.length > MAX_CONTEXT_LENGTH) {
            return Response.json({ error: `context exceeds maximum length of ${MAX_CONTEXT_LENGTH}` }, { status: 400, headers });
        }
